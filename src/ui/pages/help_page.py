...
from src.managers.notification_system import show_success, show_info, show_error
from src.utils.translator import tr
from src.ui.widgets.file_logger import clear_all_logs, get_latest_log_content, log_info
...
class DiagnosticsWorker(QThread):
    ...
    def collect_modules_info(self):
        ...
        try:
            ...
            modules.append({
                ...
                "status": "missing",
                "details": "Cannot list installed packages."
            })
    ...
    def collect_logs_info(self):
        ...
        try:
            ...
            # البحث عن ملفات السجل (بجميع الامتدادات)
            try:
                log_extensions = [".log", ".txt", ".out", ".err"]
                log_files = [f for f in os.listdir(log_dir) 
                            if (any(f.lower().endswith(ext) for ext in log_extensions) or 
                                "log" in f.lower()) and os.path.isfile(os.path.join(log_dir, f))]
            except:
                log_files = []
...
class HelpStepIndicator(QWidget):
    ...
    def setup_ui(self):
        ...
        make_theme_aware(self.slider_indicator, "slider_indicator")
...
class HelpPage(QWidget):
    ...
    def setup_ui(self):
        ...
        from src.ui.widgets.ui_helpers import create_title
        ...
        make_theme_aware(self.tabs_container, "widget")
    ...
    def setup_help_tab(self):
        ...
        make_theme_aware(content_widget, "widget")
        ...
        updates_frame.setFrameStyle(QFrame.Box)
        make_theme_aware(updates_frame, "frame")
        updates_layout = QVBoxLayout(updates_frame)
        
        updates_title = QLabel(tr("updates_section"))
        make_theme_aware(updates_title, "subtitle_text")
        updates_layout.addWidget(updates_title)
        
        # أزرار التحديث
        updates_buttons = QHBoxLayout()
        
        self.check_updates_btn = QPushButton(tr("check_for_updates"))
        make_theme_aware(self.check_updates_btn, "button")
        self.check_updates_btn.clicked.connect(self.check_for_updates)
        updates_buttons.addWidget(self.check_updates_btn)
        
        self.download_update_btn = QPushButton(tr("download_update"))
        make_theme_aware(self.download_update_btn, "button")
        self.download_update_btn.clicked.connect(self.download_update)
        self.download_update_btn.setEnabled(False)
        updates_buttons.addWidget(self.download_update_btn)
        
        updates_buttons.addStretch()
        updates_layout.addLayout(updates_buttons)
        
        # معلومات التحديث
        self.update_info_label = QLabel(tr("click_check_updates"))
        make_theme_aware(self.update_info_label, "label")
        updates_layout.addWidget(self.update_info_label)
        
        layout.addWidget(updates_frame)

        # تطبيق السمة وتحميل المحتوى بشكل متأخر لتسريع العرض الأولي
        QTimer.singleShot(150, lambda: [
            make_theme_aware(self.help_content_browser, "text_browser"),
    ...
    def setup_notification_center_tab(self):
        ...
        make_theme_aware(self.notification_center_title, "title_text")
        layout.addWidget(self.notification_center_title)

        # جلب مركز الإشعارات الفعلي
        try:
            from src.managers.notification_system import NotificationCenter
            self.notification_center = NotificationCenter(self.parent())
            make_theme_aware(self.notification_center, "notification_center")
            ...
            make_theme_aware(error_widget, "error_label")
    ...
    def setup_notification_settings_tab(self):
        ...
        make_theme_aware(self.notification_settings_title, "title_text")
        layout.addWidget(self.notification_settings_title)

        # جلب إعدادات الإشعارات الفعلية
        try:
            from src.ui.widgets.notification_settings import NotificationSettingsWidget
            self.notification_settings = NotificationSettingsWidget(self.parent())
            make_theme_aware(self.notification_settings, "widget")
        ...
        make_theme_aware(text_animation_title, "title_text")
        ...
        # خيار تفعيل/تعطيل حركة النص مع Toggle Switch
        animation_label = QLabel(tr("enable_text_animation"))
        make_theme_aware(animation_label, "label")
        controls_layout.addWidget(animation_label)
        
        from src.ui.widgets.toggle_switch import ToggleSwitch
        self.enable_text_animation = ToggleSwitch()
        ...
        # Force repaint after widget is added to layout
        QTimer.singleShot(50, self.enable_text_animation.update)

        # فاصل بصري
        separator = QFrame()
        separator.setFrameShape(QFrame.VLine)
        separator.setFrameShadow(QFrame.Sunken)
        make_theme_aware(separator, "separator")
        controls_layout.addWidget(separator)

        # خيار سرعة حركة النص
        speed_label = QLabel(tr("text_animation_speed"))
        make_theme_aware(speed_label, "label")
        ...
        make_theme_aware(self.text_animation_speed, "spin_box")
        ...
        make_theme_aware(ms_label, "label")
        ...
        self.preview_button = create_action_button("play")
        self.preview_button.setToolTip(tr("preview_text_animation"))
    ...
    def setup_diagnostics_tab(self):
        ...
        try:
            ...
            make_theme_aware(self.system_info_btn, "tab_button")
            make_theme_aware(self.performance_btn, "tab_button")
            make_theme_aware(self.modules_btn, "tab_button")
            make_theme_aware(self.logs_btn, "tab_button")
            ...
            make_theme_aware(self.diag_content, "widget")
            ...
            make_theme_aware(error_label, "error_label")
    ...
    def create_system_info_tab(self):
        ...
        make_theme_aware(self.system_tree, "tree_widget")
    ...
    def create_modules_tab(self):
        ...
        make_theme_aware(self.modules_tree, "tree_widget")
        make_theme_aware(self.modules_tree.verticalScrollBar(), "scrollbar")
        make_theme_aware(self.modules_tree.horizontalScrollBar(), "scrollbar")
        layout.addWidget(self.modules_tree)
        self.install_button = QPushButton(tr("install_requirements"))
        make_theme_aware(self.install_button, "button")
    ...
    def create_logs_tab(self):
        ...
        # قسم إعدادات التنظيف التلقائي
        cleanup_frame = QFrame()
        cleanup_frame.setFrameStyle(QFrame.Box)
        make_theme_aware(cleanup_frame, "frame")
        cleanup_layout = QVBoxLayout(cleanup_frame)
        
        cleanup_title = QLabel(tr("auto_cleanup_settings"))
        make_theme_aware(cleanup_title, "subtitle_text")
        ...
        from src.ui.widgets.toggle_switch import ToggleSwitch
        cleanup_enabled_label = QLabel(tr("enable_auto_cleanup"))
        make_theme_aware(cleanup_enabled_label, "label")
        cleanup_controls.addWidget(cleanup_enabled_label)
        
        self.cleanup_enabled = ToggleSwitch()
        self.cleanup_enabled.setChecked(get_setting("auto_cleanup_enabled", True))
        self.cleanup_enabled.stateChanged.connect(self.save_cleanup_settings)
        cleanup_controls.addWidget(self.cleanup_enabled)
        
        # فترة التنظيف
        interval_label = QLabel(tr("cleanup_interval"))
        make_theme_aware(interval_label, "label")
        cleanup_controls.addWidget(interval_label)
        
        self.cleanup_interval = QSpinBox()
        self.cleanup_interval.setRange(1, 60)
        self.cleanup_interval.setValue(get_setting("cleanup_interval_minutes", 5))
        self.cleanup_interval.setSuffix(" " + tr("minutes"))
        make_theme_aware(self.cleanup_interval, "spin_box")
        self.cleanup_interval.valueChanged.connect(self.save_cleanup_settings)
        cleanup_controls.addWidget(self.cleanup_interval)
        
        cleanup_controls.addStretch()
        
        # زر تنظيف يدوي
        manual_cleanup_btn = QPushButton(tr("manual_cleanup"))
        make_theme_aware(manual_cleanup_btn, "button")
        manual_cleanup_btn.clicked.connect(self.manual_cleanup)
        cleanup_controls.addWidget(manual_cleanup_btn)
        ...
        make_theme_aware(refresh_logs_button, "button")
        ...
        make_theme_aware(clear_logs_button, "button")
        ...
        make_theme_aware(retention_label, "label")
        ...
        make_theme_aware(self.retention_combo, "combo")
        ...
        make_theme_aware(self.logs_text, "text_edit")

        # تطبيق نظام السمات المدمج على أشرطة التمرير
        make_theme_aware(self.logs_text.verticalScrollBar(), "scrollbar")
        make_theme_aware(self.logs_text.horizontalScrollBar(), "scrollbar")
    ...
    def _on_diagnostics_complete(self):
        ...
        from src.managers.notification_system import show_success
    ...
    def _style_diag_tab_button(self, btn, is_active=False):
        ...
        try:
            ...
            make_theme_aware(temp_btn, "tab_button")
    ...
    def update_modules_info(self, modules):
        ...
        if hasattr(self, "modules_tree") and self.modules_tree:
            ...
            for module_info in modules:
                ...
                translated_status = tr(status, default_value=status)
                item = QTreeWidgetItem(self.modules_tree, [name, translated_status, details])
    ...
    def export_report(self):
        from src.utils.diagnostics import export_diagnostics
        content = []
    ...
    def on_installation_finished(self, success, output):
        from src.managers.notification_system import show_success, show_error
        if success:
    ...
    def handle_clear_logs(self):
        ...
        if reply == QMessageBox.Yes:
            ...
            try:
                ...
                if not os.path.exists(log_dir):
                    from src.managers.notification_system import show_info
                    show_info(tr("no_log_files_found"))
                    self.refresh_logs_info()
                    return

                # حذف جميع ملفات السجل (بجميع الامتدادات)
                deleted_count = 0
                log_extensions = [".log", ".txt", ".out", ".err"]
                
                for filename in os.listdir(log_dir):
                    # حذف الملفات التي تحتوي على كلمة log أو لها امتدادات السجل
                    if (any(filename.lower().endswith(ext) for ext in log_extensions) or 
                        "log" in filename.lower()):
                        ...
                        try:
                            if os.path.isfile(file_path):
                                ...
                                # حذف مجلدات السجل الفرعية أيضاً
                                import shutil
                ...
                from src.managers.notification_system import show_error
    ...
    def save_cleanup_settings(self):
        """حفظ إعدادات التنظيف التلقائي"""
        enabled = self.cleanup_enabled.isChecked()
        interval = self.cleanup_interval.value()
        
        set_setting("auto_cleanup_enabled", enabled)
        set_setting("cleanup_interval_minutes", interval)
        
        # تطبيق الإعدادات على المنظف الخلفي
        try:
            from src.utils.background_cleaner import background_cleaner
            if enabled:
                background_cleaner.interval = interval * 60  # تحويل إلى ثوان
                if not background_cleaner.running:
        ...
        from src.managers.notification_system import show_success
        show_success(tr("cleanup_settings_saved"))
    
    def manual_cleanup(self):
        """تنظيف يدوي للملفات المؤقتة"""
        try:
            from src.utils.temp_cleaner import clean_temp_files
            cleaned_files, cleaned_size = clean_temp_files()
            
            if cleaned_files > 0:
                size_mb = cleaned_size / (1024 * 1024)
                from src.managers.notification_system import show_success
                show_success(tr("manual_cleanup_success", files=cleaned_files, size=f"{size_mb:.2f}"))
            else:
                from src.managers.notification_system import show_info
                show_info(tr("no_temp_files_found"))
        except Exception as e:
            from src.managers.notification_system import show_error
            show_error(tr("manual_cleanup_error", error=str(e)))

    def check_for_updates(self):
        """فحص التحديثات المتاحة"""
        self.check_updates_btn.setEnabled(False)
        self.check_updates_btn.setText(tr("checking_updates"))
        self.update_info_label.setText(tr("checking_for_updates"))
        
        try:
            from src.utils.update_checker import check_for_updates
            is_available, latest_version = check_for_updates()
            
            if is_available:
                self.latest_version = latest_version
                # الحصول على رابط التحميل
                self.download_url = f"https://github.com/aljawalcar-glitch/ApexFlow/releases/tag/{latest_version}"
                
                self.update_info_label.setText(tr("update_available", version=latest_version))
                self.download_update_btn.setEnabled(True)
                
                from src.managers.notification_system import show_success
                show_success(tr("update_found", version=latest_version))
            else:
                self.update_info_label.setText(tr("no_updates_available"))
                self.download_update_btn.setEnabled(False)
                
                from src.managers.notification_system import show_info
                show_info(tr("app_up_to_date"))
                
        except Exception as e:
            self.update_info_label.setText(tr("error_checking_updates", error=str(e)))
            self.download_update_btn.setEnabled(False)
            
            from src.managers.notification_system import show_error
            show_error(tr("update_check_failed"))
        
        finally:
            self.check_updates_btn.setEnabled(True)
            self.check_updates_btn.setText(tr("check_for_updates"))
    ...
    def download_update(self):
        ...
        try:
            ...
            from src.managers.notification_system import show_info
            show_info(tr("opening_download_page"))
            
        except Exception as e:
            from src.managers.notification_system import show_error
            show_error(tr("failed_to_open_download", error=str(e)))
