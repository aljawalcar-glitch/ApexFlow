...
from PySide6.QtWidgets import QApplication, QVBoxLayout, QHBoxLayout, QPushButton, QWidget, QGroupBox, QMessageBox
from src.ui.widgets.base_page import BasePageWidget
from src.ui.widgets.ui_helpers import create_button, create_title
...
class ConvertPage(BasePageWidget):
    ...
    def __init__(self, file_manager, operations_manager, notification_manager, parent=None):
        ...
        # Create a container for all options, similar to CompressPage
        self.options_container = QWidget()
        self.options_layout = QVBoxLayout(self.options_container)
        self.options_layout.setContentsMargins(0, 15, 0, 0)
        self.options_layout.setSpacing(10)
        self.main_layout.insertWidget(1, self.options_container) # Insert after header

        # Create the path and execute section inside the options container
        self.create_path_and_execute_section(self.options_layout)

        # Add file list frame directly to the main layout
    ...
    def add_files(self, files):
        ...
        if len(filtered_files) < original_file_count:
    ...
    def create_header_layout(self):
        ...
        for key, text in self.convert_options.items():
            ...
            make_theme_aware(button, "tab_button")
        ...
        if language_manager.is_rtl():  # العربية: العنوان يمين، الأزرار يسار
        ...
        make_theme_aware(self.add_files_btn, "special_button")
        self.cancel_btn = create_button(tr("cancel_operation"), on_click=self.full_reset)
        ...
        alignment = Qt.AlignRight if language_manager.is_rtl() else Qt.AlignLeft
        if language_manager.is_rtl():  # العربية RTL: إلغاء ← إضافة ملفات
            ...
            self.controls_layout.addWidget(self.add_files_btn)
            self.controls_layout.addWidget(self.cancel_btn)
        self.controls_layout.setAlignment(alignment)
    ...
    def create_path_and_execute_section(self, parent_layout):
        ...
        from src.ui.widgets.svg_icon_button import SVGIconButton, create_action_button
        ...
        self.change_path_btn = create_colored_icon_button("folder-open", 22, "", tr("change_save_path_tooltip"))
        ...
        self.execute_btn = create_colored_icon_button("play", 24, "", tr("execute_tooltip"))
    ...
    def on_tab_selected(self):
        ...
        # إذا كان المستخدم ينقر على التبويب النشط بالفعل، لا تفعل شيئًا
        if sender.objectName() == self.active_operation:
            sender.setChecked(True)
            return

        # التحقق من وجود عمل غير مكتمل، تمامًا مثل النافذة الرئيسية
        if self.file_list_frame.get_files():
            reply = QMessageBox.warning(
                self,
                tr("unsaved_work_title"),
                tr("unsaved_tab_switch_warning"), # استخدام الرسالة المخصصة للتبويبات
                QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
                QMessageBox.StandardButton.No
            )

            if reply == QMessageBox.StandardButton.No:
                # إذا رفض المستخدم، أعد تحديد التبويب القديم وألغِ الجديد
                if self.active_operation and self.active_operation in self.top_buttons:
                    self.top_buttons[self.active_operation].setChecked(True)
        ...
        if not hasattr(self, 'is_initializing') or not self.is_initializing:
            self.options_container.show() # Show options container instead of workspace
            self.add_files_btn.show()
            self.cancel_btn.show()
    ...
    def full_reset(self):
        """إعادة تعيين كاملة للواجهة بما في ذلك التبويبات."""
        self.reset_ui(reset_tabs=True)
    ...
    def reset_ui(self, reset_tabs=True):
        ...
        # إخفاء عناصر التحكم ومنطقة العمل
        ...
        # مسح قائمة الملفات وإخفائها
        if self.file_list_frame.get_files():
            self.file_list_frame.clear_all_files()
        self.file_list_frame.hide()

        # إعادة تعيين التبويبات إذا طُلب ذلك
        if reset_tabs:
            self.active_operation = ""  # إلغاء تحديد أي عملية نشطة
            for button in self.top_buttons.values():
                button.setChecked(False)
            
            # إعادة تعيين نص زر إضافة الملفات إلى حالته الافتراضية
