...
from src.ui.widgets.ui_helpers import create_button
from src.utils.translator import tr
from src.ui.widgets.smart_drop_overlay import SmartDropOverlay
...
class MergePage(BasePageWidget):
    ...
    def __init__(self, file_manager, operations_manager, notification_manager, parent=None):
        ...
        print_toggle_layout = QHBoxLayout()
        print_label = QLabel(tr("print_after_merge"))
        from src.ui.widgets.toggle_switch import ToggleSwitch
        self.print_after_merge = ToggleSwitch()
        print_toggle_layout.addWidget(print_label)
        print_toggle_layout.addStretch()
        print_toggle_layout.addWidget(self.print_after_merge)
        print_options_layout.addLayout(print_toggle_layout)
        ...
        self.merge_button = create_colored_icon_button("merge", 24, "", tr("merge_files_tooltip"))
        ...
        action_layout.addWidget(self.print_options_frame, 2)
        action_layout.addWidget(self.merge_button_frame, 1)
    ...
    def add_folder(self):
        ...
        from src.utils.app_utils import browse_folder_simple
    ...
    def update_merge_button_text(self):
        ...
        if print_after_merge and files_count == 1:
            ...
            self.merge_button.setToolTip(tr("print_files"))
            from src.ui.widgets.svg_icon_button import create_colored_icon
            colored_icon = create_colored_icon("printer", 24)
            if colored_icon:
                self.merge_button.setIcon(colored_icon)
        elif print_after_merge:
            # حالة الدمج والطباعة
            self.merge_button_frame.setTitle(tr("merge_and_print_files"))
            self.merge_button.setToolTip(tr("merge_and_print_files"))
            from src.ui.widgets.svg_icon_button import create_colored_icon
            colored_icon = create_colored_icon("merge", 24)
            if colored_icon:
                self.merge_button.setIcon(colored_icon)
        else:
            # حالة الدمج فقط
            self.merge_button_frame.setTitle(tr("merge_files"))
            self.merge_button.setToolTip(tr("merge_files"))
            from src.ui.widgets.svg_icon_button import create_colored_icon
            colored_icon = create_colored_icon("merge", 24)
            if colored_icon:
                self.merge_button.setIcon(colored_icon)
    ...
    def reset_ui(self):
        ...
        # إخفاء عناصر التحكم في الإجراءات
        self.action_widget.setVisible(False)
        self.print_after_merge.setChecked(False)

        # التأكد من إخفاء إطار الملفات
