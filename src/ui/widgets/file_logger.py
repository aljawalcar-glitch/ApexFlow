...
class ApexFlowFileLogger:
    ...
    def setup_logger(self):
        ...
        # تسجيل معلومات عن ملف السجل فقط إذا كان الملف جديد
        if not os.path.exists(log_file):
            self.logger.info(f"تم إنشاء ملف السجل: {log_file}")
        else:
            # إضافة فاصل للجلسة الجديدة
...
# إنشاء مثيل مشترك (تأخير التهيئة)
file_logger = None

def get_file_logger():
    """الحصول على مثيل مسجل الملفات مع التهيئة المتأخرة"""
    global file_logger
...
def log_warning(message):
    """رسالة تحذير"""
    get_file_logger().warning(message)
...
def log_critical(message):
    """رسالة خطأ حرج"""
    get_file_logger().critical(message)
